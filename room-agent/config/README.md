...
这个配置既用于生成 beacon 固件代码，也作为启动时向 qwen-backend 注册空间绑定的来源。

```yaml
# BLE Beacon配置（空间感知层）
beacon:
  enabled: true # 是否启用BLE Beacon广播
  beacon_id: "2" # qwen-backend 中登记的 Beacon 唯一标识，当前等于 major 字符串
  major: 2 # Room identifier (2=卧室, 1=客厅, 3=书房...)
  minor: 0 # Zone/Position in room (0-65535)
  measured_power: -59 # Calibrated RSSI at 1 meter
...
定义了当前 Room Agent 的身份。其中 `room_id` 是后续房间绑定、Beacon 映射和后端登记时的核心字段。
...
- `beacon_id`: 后端登记和 Personal Agent 查询使用的 Beacon 标识，当前等于 `major` 转成的字符串
- `major`: 房间编号
- `minor`: 房间内区域编号
...
- `beacon_id` 和 Personal Agent 扫描后生成的标识一致
- 功率、间隔等参数一致
