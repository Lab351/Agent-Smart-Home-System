...
## 任务
根据用户输入和当前上下文，把输入归类成以下两种结果之一：
1. chat：简单交流、寒暄、解释、介绍、开放式问答，后端直接返回回复
2. agent_message：凡是需要把用户原话转发给 room-agent 或 home-agent 处理的请求
...
返回 JSON，格式如下：
{
  ...
  "query": {
    "type": "room_devices | room_state，仅当你能明确识别为房间设备/状态查询时填写，否则为 null",
    "room_id": "房间ID，不确定时为 null",
    "reason": "为什么判定为该查询"
  },
  "intent": {
    "device": "设备ID（从可用设备中选择，不确定则为 null）",
    "action": "动作ID，仅当你能明确识别控制意图时填写，否则为 null",
    "parameters": {}
  },
  "routing": {
...
## 关键规则
1. 简单聊天、介绍、解释、问答，返回 kind="chat"
2. 只要需要把话转给 agent 继续处理，无论是查询、控制还是复杂请求，都返回 kind="agent_message"
3. 像“房间里有什么设备”“有哪些能力”“客厅灯现在开着吗”这类请求，必须返回 kind="agent_message"，并尽量补充 query.type
4. chat 时 routing.target 必须为 null
5. agent_message 时：
   - 如果是设备/能力清单查询，query.type = "room_devices"
   - 如果是当前状态查询，query.type = "room_state"
   - 如果是明确控制，intent.device / intent.action 尽量填写
6. 路由规则：
   - 当前房间内的查询或控制，优先 target = "room-agent"
   - 跨房间、全局场景或无法直接落到单个 room-agent 的请求，target = "home-agent"
   - 如果当前房间上下文足够，优先把单房间请求发给 room-agent
...
export class IntentService {
  ...
  private parseResponse(response: string): IntentParseResult | null {
    try {
      ...
      const query =
        parsed.query?.type === 'room_state' || parsed.query?.type === 'room_devices'
          ? {
      ...
      const intent =
        parsed.intent && (this.resolveString(parsed.intent.action) || this.resolveString(parsed.intent.device))
          ? {
      ...
      return {
        ...
        routing: this.normalizeRouting(
          parsed.routing,
          kind,
          query?.reason || this.resolveString(parsed.routing?.reason) || null,
          query?.room_id || null,
          intent,
        ),
        raw_response: response,
  ...
  private normalizeKind(value: any): IntentParseResult['kind'] | null {
    if (value?.kind === 'chat' || value?.kind === 'agent_message') {
      return value.kind;
    }

    if (value?.kind === 'query' || value?.kind === 'action') {
    ...
    if (
      value?.intent?.action ||
      value?.intent?.device ||
      value?.routing?.target === 'room-agent' ||
      value?.routing?.target === 'home-agent'
    ) {
      return 'agent_message';
    }
  ...
  private normalizeRouting(
    ...
    return {
      target:
        routing?.target === 'room-agent' || routing?.target === 'home-agent'
          ? routing.target
          : fallbackRoomId || intent?.action || intent?.device
            ? 'room-agent'
            : 'home-agent',
