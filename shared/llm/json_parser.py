...
async def parse_json_with_repair(
    raw_text: str,
    *,
    llm_provider: JsonRepairProvider | None = None,
    ...
    if llm_provider is None:
        raise JsonRepairError("JSON parsing failed and no LLM repair provider was supplied.") from last_error

    repaired_text = await llm_provider.complete_text(
    ...
    raise JsonRepairError("LLM JSON repair failed to produce valid JSON.") from last_error
...
def _build_repair_messages(
    ...
    *,
    ...
    schema_text = json.dumps(schema, ensure_ascii=False, indent=2) if schema is not None else "无 schema 约束"
    return [
        {
            "role": "system",
            "content": (
